...
が、読み込む画像データが大きくなると、読み込みが完了する前にレンダリングされてしまうため、描画されない。（レンダリング時には img が null となるため。）
...
// 描画したいsvg画像の中身をコピー。動的に変更したい部分だけpropsで受け取るように適宜加工する。サイズを変更しても表示が崩れないように上手いこと調整する必要はあり。
